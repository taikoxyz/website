...
Taiko is a fully permissionless and decentralized Ethereum-equivalent ZK-Rollup. This means using Taiko is exactly the same as using Ethereum. Not only does Taiko support all EVM opcodes, but it also supports the full JSON-RPC execution client API.
...
A based rollup is a term coined by Justin Drake, to which the original definition can be read [here](https://ethresear.ch/t/based-rollups-superpowers-from-l1-sequencing/15016).
...
The different types of ZK-EVMs make tradeoffs between compatibility and proof generation costs. A Type 1 ZK-EVM prioritizes compatibility over proof generation cost.
...
Your Sepolia archive node should be fine. For your Taiko node, you should remove the node (see the [node runner manual](/docs/manuals/node-runner-manual)) and directory, update and then follow the [Taiko node running guide](/docs/guides/run-a-taiko-node) again.
